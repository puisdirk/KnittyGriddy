import 'package:flutter/material.dart';
import 'package:knitty_griddy/controls/stitch_icon.dart';
import 'package:knitty_griddy/model/knitting_symbol_path.dart';
import 'package:knitty_griddy/model/knitting_symbol_paths.dart';
import 'package:knitty_griddy/model/knitty_griddy_model.dart';
import 'package:knitty_griddy/stitchrepo/knitting_symbol_path_control.dart';
import 'package:knitty_griddy/stitchrepo/stitch_definition.dart';
import 'package:provider/provider.dart';

class StitchPathChooser extends StatefulWidget {
  const StitchPathChooser({
    super.key
  });

  @override
  State<StitchPathChooser> createState() => _StitchPathChooserState();
}

class _StitchPathChooserState extends State<StitchPathChooser> {

  List<StitchDefinition> _selectedStitches = [];
  List<KnittingSymbolPath> _selectedPaths = [];
  
  static const double _spacerwidth = 14;
  static const double _iconWidth = 16;

  void _togglePath(KnittingSymbolPath path) {
    if (_selectedPaths.contains(path)) {
      setState(() {
        _selectedPaths = _selectedPaths.where((p) => p != path).toList();
      });
    } else {
      setState(() {
        _selectedPaths = [..._selectedPaths, path];
      });
    }
  }

  void _toggleStitch(StitchDefinition def) {
    if (_selectedStitches.contains(def)) {
      setState(() {
        _selectedStitches = _selectedStitches.where((s) => s != def).toList();
      });
    } else {
      setState(() {
        _selectedStitches = [..._selectedStitches, def];
      });
    }
  }

  List<List<KnittingSymbolPath>> get _mergedSymbols {
    if (_selectedStitches.isEmpty && _selectedPaths.isEmpty) {
      return [];
    }

    if (_selectedStitches.isEmpty) {
      return [_selectedPaths];
    }

    int width = 0;
    for (StitchDefinition def in _selectedStitches) {
      if (def.columns > width) {
        width = def.columns;
      }
    }
    List<List<KnittingSymbolPath>> res = List.generate(width, (idx) => []);

    res[0].addAll(_selectedPaths);

    for (StitchDefinition def in _selectedStitches) {
      for (int idx = 0; idx < def.columns; idx++) {
        res[idx].addAll(def.symbolAt(idx).paths);
      }
    }

    return res;
  }

  Widget createStandardPathsCategory() {
    double widestSymbolPath = 0;
    for (KnittingSymbolPath path in KnittingSymbolPaths.paths) {
      double width = (_spacerwidth * 4) + (path.name.length * 10) + _iconWidth;
      if (width > widestSymbolPath) {
        widestSymbolPath = width;
      }
    }

    List<Widget> cards = [];
    for (KnittingSymbolPath path in KnittingSymbolPaths.paths) {
      cards.add(SizedBox(
        width: widestSymbolPath, 
        height: 50,
        child: Card(
          color: _selectedPaths.contains(path) ? Colors.blue.withAlpha(60) : null,
          child: InkWell(
            borderRadius: const BorderRadius.all(Radius.circular(10)),
            splashColor: Colors.black.withAlpha(30),
            onTap: () => _togglePath(path),
            child: Row(
              children: [
                const SizedBox(width: _spacerwidth,),
                KnittingSymbolPathControl(knittingSymbolPath: path, iconSize: _iconWidth,),
                const SizedBox(width: _spacerwidth,),
                Text(path.name),
                const SizedBox(width: _spacerwidth,)
              ],
            ),
          ),
        ),
      ));
    }

    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        const Text('Basic symbols'),
        Wrap(children: cards,),
      ],
    );
  }

  Widget createCategory(MapEntry<String, List<StitchDefinition>> entry) {
    double widestStitchWidth = 0;
    for (StitchDefinition def in entry.value) {
      double width = (_spacerwidth * 3) + (def.columns * 28.0) + (def.name.length * 10);
      if (width > widestStitchWidth) {
        widestStitchWidth = width;
      }
    }

    List<Widget> cards = [];
    for (StitchDefinition sd in entry.value) {
      bool stitchSelected = _selectedStitches.contains(sd);

      cards.add(SizedBox(
        width: widestStitchWidth, 
        height: 50,
        child: Card(
          color: stitchSelected ? Colors.blue.withAlpha(60) : null,
          child: InkWell(
            borderRadius: const BorderRadius.all(Radius.circular(10)),
            splashColor: Colors.blue.withAlpha(30),
            onTap: () => _toggleStitch(sd),
            child: Row(
              children: [
                const SizedBox(width: _spacerwidth,),
                StitchIcon(stitchDefinition: sd, iconSize: _iconWidth),
                const SizedBox(width: _spacerwidth,),
                Text(sd.name),
                const SizedBox(width: _spacerwidth,),
              ],
            ),
          ),
        ),
      ));
    }
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start, 
      children: [
        Text(entry.key), 
        Wrap(children: cards,)
      ],
    );
  }


  @override
  Widget build(BuildContext context) {
    return AlertDialog(
      title: const Text('Select paths'),
      content: SizedBox(
        width: 600,
        height: 400,
        child: Selector<KnittyGriddyModel, Map<String, List<StitchDefinition>>>(
          selector: (_, model) => model.selectStitchDefinitionsPerCategory(''),
          builder: (context, stitchDefinitions, _) {
           return ListView(
              children: [
                createStandardPathsCategory(),
                for (MapEntry<String, List<StitchDefinition>> entry in stitchDefinitions.entries)
                  createCategory(entry),
              ],
            );
          }
        ),
      ),
      actions: [
        ElevatedButton.icon(
          onPressed: () => Navigator.of(context).pop(<List<KnittingSymbolPath>>[]), 
          label: const Text('Cancel'),
          icon: const Icon(Icons.cancel_outlined),
        ),
        ElevatedButton.icon(
          onPressed: (_selectedPaths.isEmpty && _selectedStitches.isEmpty) ? null : () => Navigator.of(context).pop(_mergedSymbols), 
          label: const Text('OK'),
          icon: const Icon(Icons.check),
        )
      ],
    );
  }
}